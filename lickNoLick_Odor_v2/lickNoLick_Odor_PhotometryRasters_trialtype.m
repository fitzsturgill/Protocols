function lickNoLick_Odor_PhotometryRasters(Op, varargin)
    ...
    switch Op
        case 'init' 
            ...
            BpodSystem.ProtocolFigures.phRaster.trialsToPlot = ls.trialsToPlot;
        ...
        case 'update'
            ...
            trialsToPlot = BpodSystem.ProtocolFigures.phRaster.trialsToPlot;
            ...
            for i = 1:length(trialsToPlot)
                thisTrialIndex = trialsToPlot(i);
                outcome = onlineFilterTrials_v2('TrialTypes', thisTrialIndex); % miss or false alarm            
                %outcome_right = onlineFilterTrials_v2('OdorValveIndex', thisTrialIndex,'TrialOutcome', [1, 2]); % hit or correct rejection
%                 SrewardTrials = onlineFilterTrials_v2('ReinforcementOutcome', 'RewardS');
%                 BrewardTrials = onlineFilterTrials_v2('ReinforcementOutcome', 'RewardB');
...
%                 punishTrials = onlineFilterTrials_v2('ReinforcementOutcome', {'Punish', 'WNoise'});                
                if BpodSystem.Data.Settings.GUI.LED1_amp > 0
                    channelData = BpodSystem.PluginObjects.Photometry.trialDFF{1}(outcome);
...
%                     CData(outcome_left, (1:nSamples)) = fliplr(channelData(outcome_left, :));
                     CData(outcome, (nSamples+1):end) = channelData;
%                     % add color tags marking trial reinforcment outcome
%                     % high color = reward, 0 color = neutral, low color = punish
%                     CData(SrewardTrials & outcome_left, (nSamples - phRStamp + 1):nSamples) = 200; %
%                     CData(BrewardTrials & outcome_left, (nSamples - phRStamp + 1):nSamples) = 255; %255 is arbitrary large value that will max out color table
...
%                     CData(SrewardTrials & outcome_right, (nSamples+1):(nSamples + phRStamp)) = 255; 
%                     CData(BrewardTrials & outcome_right, (nSamples+1):(nSamples + phRStamp)) = 200; % 255 is arbitrary large value that will max out color table
...
%                     CData(punishTrials & outcome_right, (nSamples+1):(nSamples + phRStamp)) = -255;            
                ...
                if BpodSystem.Data.Settings.GUI.LED2_amp > 0
                    channelData = BpodSystem.PluginObjects.Photometry.trialDFF{2}(outcome);
                    nSamples = size(channelData, 2);
                    if i == 1
...
%                     CData(neutralTrials & outcome_left, (nSamples - phRStamp + 1):nSamples) = 0;            
%                     CData(punishTrials & outcome_left, (nSamples - phRStamp + 1):nSamples) = -255; 
%                     CData(SrewardTrials & outcome_right, (nSamples+1):(nSamples + phRStamp)) = 200;
%                     CData(BrewardTrials & outcome_right, (nSamples+1):(nSamples + phRStamp)) = 255; % 255 is arbitrary large value that will max out color table
%                     CData(neutralTrials & outcome_right, (nSamples+1):(nSamples + phRStamp)) = 0;            
%                     CData(punishTrials & outcome_right, (nSamples+1):(nSamples + phRStamp)) = -255;                   
